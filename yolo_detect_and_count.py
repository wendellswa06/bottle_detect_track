...
class YOLOv8_ObjectDetector:
    ...
    def custom_display(self, colors, show_cls = True, show_conf = True):
        ...
        for box in self.results.boxes:
            ...
            x1 , y1 , x2, y2 = np.squeeze(box.xyxy.cpu().numpy()).astype(int)#for gpu case
    ...
    Returns:
        ...
        while cap.isOpened():
            ...
            for box in results.boxes:
                ...
                # x1 , y1 , x2, y2 = np.squeeze(box.xyxy.numpy()).astype(int)
                x1 , y1 , x2, y2 = np.squeeze(box.xyxy.cpu().numpy()).astype(int)#for gpu
            ...
            # frame = cv2.putText(frame,f"FPS : {fps:,.2f}" , 
            #                     (25,100), cv2.FONT_HERSHEY_COMPLEX, 
            #                 4,  (0,255,255), 1, cv2.LINE_AA)
            
            # Display Counting results
            count_txt = f"TOTAL COUNT : {len(totalCount)}"
            frame = cv2.putText(frame, count_txt, (25,250), cv2.FONT_HERSHEY_COMPLEX, 4, (0, 0, 255), 2)
